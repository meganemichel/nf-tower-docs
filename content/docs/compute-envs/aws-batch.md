...
**6.** Choose JSON and copy the contents of the [policy linked above](https://github.com/seqeralabs/nf-tower-aws/blob/master/forge/forge-policy.json).
...
You can choose to create a compute environment that will launch either **Spot** or **On-demand** instances. Spot instances can cost as little as 20% of on-demand instances and with Nextflow's ability to automatically relaunch failed tasks, Spot is almost always the recommended provisioning model. It is important to note that when selecting Spot, Tower Forge will create two queues in your AWS environment. The first or **Head queue** will be an On-demand instance and will be used for the Nextflow runtime job. The second or **Compute queue** will be used for pipeline tasks.
{{% /tip %}}
...
**7.** Enter the **Head queue** which is the name of the AWS Batch queue that the Nextflow runtime job will run and a **Compute queue** which is the name of the AWS Batch queue that tasks will be submitted to.
